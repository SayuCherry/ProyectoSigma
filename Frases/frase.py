...
def obtenerAngulos(results, width, height):
    ...
    for hand_landmarks in results.multi_hand_landmarks:
        coords = {
            "dedo meñique": [(mp_hands.HandLandmark.PINKY_TIP, mp_hands.HandLandmark.PINKY_PIP, mp_hands.HandLandmark.PINKY_MCP)],
            "dedo anular": [(mp_hands.HandLandmark.RING_FINGER_TIP, mp_hands.HandLandmark.RING_FINGER_PIP, mp_hands.HandLandmark.RING_FINGER_MCP)],
            "dedo medio": [(mp_hands.HandLandmark.MIDDLE_FINGER_TIP, mp_hands.HandLandmark.MIDDLE_FINGER_PIP, mp_hands.HandLandmark.MIDDLE_FINGER_MCP)],
            "dedo índice": [(mp_hands.HandLandmark.INDEX_FINGER_TIP, mp_hands.HandLandmark.INDEX_FINGER_PIP, mp_hands.HandLandmark.INDEX_FINGER_MCP)],
            "pulgar exterior": [(mp_hands.HandLandmark.THUMB_TIP, mp_hands.HandLandmark.THUMB_IP, mp_hands.HandLandmark.THUMB_MCP)],
            "pulgar interior": [(mp_hands.HandLandmark.THUMB_TIP, mp_hands.HandLandmark.THUMB_MCP, mp_hands.HandLandmark.WRIST)]
        }
...
def condicionalesFrases(dedos, frame):
    try:
        font = ImageFont.truetype("DINRoundPro.ttf", 30)  # Ajusta el tamaño de la fuente aquí
    except IOError:
    ...
    frases = {
        ...
        "Frase - Bueno": [0, 1, 1, 1, 1, 1],
        }

    color_correcto = (0, 255, 0)  # Verde
    color_incorrecto = (250, 128, 114)  # Rojo
    color = color_incorrecto
    frase_detectada = 'Identificado Las frase '

    for frase, config in frases.items():
        if dedos == config:
    ...
    text_bbox = draw.textbbox((0, 0), frase_detectada, font=font)
    ...
    for offset in offsets:
        draw.text((text_position[0] + offset[0], text_position[1] + offset[1]), frase_detectada, font=font, fill=border_color)

    # Dibujar texto
    draw.text(text_position, frase_detectada, font=font, fill=(255, 255, 255, 255))
...
def generate():
    ...
    try:
        font = ImageFont.truetype("DINRoundPro.ttf", 30)
    except IOError:
        font = ImageFont.load_default()
        ...
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                if dedos == [0, 1, 0, 0, 1, 0] and abs(resta) > 30:
                    ...
                    draw.text(text_position, '', font=font, fill=(0, 0, 0, 255))
                    frame = cv2.cvtColor(np.array(frame_pil), cv2.COLOR_RGB2BGR)
                    print("movimiento")

                frame = condicionalesFrases(dedos, frame)
