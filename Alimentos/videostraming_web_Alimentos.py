...
# Función para determinar los alimentos en función de los ángulos de los dedos
def condicionalesAlimentos(dedos, frame):
    try:
    ...
    alimento_detectado = 'Identificado-alimento'

    for alimento, config in alimentos.items():
        if dedos == config:
            color = correct_color
    ...
    text_bbox = draw.textbbox((0, 0), alimento_detectado, font=font)
    ...
    for offset in offsets:
        draw.text((text_position[0] + offset[0], text_position[1] + offset[1]), alimento_detectado, font=font, fill=border_color)

    # Dibujar texto
    draw.text(text_position, alimento_detectado, font=font, fill=(255, 255, 255, 255))
    ...
    with mp_hands.Hands(
        ...
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                frame = condicionalesAlimentos(dedos, frame)
